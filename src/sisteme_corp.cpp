...
#include "celula_imunitara.h"
    ...
    SecventaADN::SecventaADN(const std::string& cod ) : cod_genetic(cod) {}
    ...
    void SecventaADN::suferaMutatie() {
        ...
        const char baze[] = {'A', 'C', 'G', 'T'};
        int index = (rand() % 4) * 2;
        cod_genetic[index] = baze[rand() % 4];
    }
CelulaImunitara* FabricaMaduva::produceCelula(const std::string& tip_celula, double oxigen_pacient) {
